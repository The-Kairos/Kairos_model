...
| **UDST Honors** | 2 h 23m | 4 h 0m* | 45.3m* | **5.1m** | **13.7m** | **~14x** | High-Quality Bilingual |
| **Web Summit Qatar** | 7 h 4m | N/A | N/A | **14.6m** | **23.4m** | **-** | Massive 7-hour stress test |
| **Learning_ SVM** | 49.6m | 35.0m* | 8.0m* | **1.6m** | **1.2m** | **~15x** | Technical Lecture |
| **DJI Chinatown Walk** | 44.2m | 4 h 0m* | 40.0m* | **1.5m** | **2.8m** | **~55x** | Street Ambience |
...
| **CCTV Dogs** | 5.0m | N/A | N/A | **0.0m** | **0.2m** | **-** | Fixed-Length Scenes |
| **Statistical Learning** | 13.6m | N/A | N/A | **0.6m** | **0.5m** | **-** | Fixed-Length Scenes |

*\* Estimated based on legacy local processing benchmarks.*
...
## Technical Note: Zero-Cut Videos (FIXED)

Previously, **CCTV Dogs** and **Statistical Learning** returned 0 scenes because they contained single continuous camera angles with no visual transitions.

**The Fix:**
We have implemented a fallback mechanism: *If PySceneDetect returns 0 scenes, the system now automatically splits the video into fixed 10-second scenes based on timestamps.* This ensures that ASR and AST have valid segments to map their results to, as seen in the updated timings above.
