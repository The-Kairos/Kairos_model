"""
audio_detector.py — 2-Stage Audio Pre-Scanner with Dynamic Thresholds
...
All thresholds are dynamically scaled by video duration.
See AUDIO_DETECTION_REFERENCE.md for the full technical reference.
"""
...
def get_dynamic_thresholds(duration_minutes: float) -> dict:
    ...
    return {
        ...
        "MIN_SILENCE_DURATION_MS":      300,          # constant
...
def compute_per_scene_rms(audio: np.ndarray, sr: int, scenes: list) -> list:
    ...
    for scene in scenes:
        ...
        chunk = audio[i0:i1]
        if len(chunk) == 0:
            per_scene.append(-200.0)  # effectively -inf
        else:
...
def scan_audio(video_path: str, scenes: list, target_sr: int = 16000, debug: bool = False) -> dict:
    ...
    Returns a decision dict usable by whisper_singlecall and ast_processor.
    """
    t_start = time.time()
