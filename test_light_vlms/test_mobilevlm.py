"""
Light VLM: MobileVLM (mobile-friendly vision-language model).
...
def load_vlm_model(model_id="mobilevllm/MobileVLM-1.7B"):
...
def caption_image(model, processor, image, prompt=None):
    ...
    # Most MobileVLM variants expect text with image tokens plus image input.
    text = f"<image>\n{prompt}"
    inputs = processor(text=text, images=[image], return_tensors="pt").to(model.device)
    with torch.no_grad():
        output = model.generate(**inputs, max_new_tokens=128)
    # Strip off prompt tokens when decoding if necessary.
