...
def parse_args() -> argparse.Namespace:
    ...
    * **rag** – start an interactive RAG session for a single video.
    ...
    process.add_argument(
        "--video", action="append", help="Blob name or path (repeatable)"
    )
    process.add_argument(
        "--all", action="store_true", help="Process all catalog videos"
    )
    process.add_argument(
        "--filter",
    ...
    process.add_argument(
        "--preset",
        choices=["default", "fast", "motion", "static"],
        default="default",
        help="Pipeline config preset",
    )
    process.add_argument(
        "--llm",
    ...
    process.add_argument(
        ...
        help="Redo a processing step (repeatable)",
    )
    process.add_argument(
        "--redo-only",
        nargs="*",
        choices=REDO_CHOICES,
        help="Redo only specified steps (no dependents)",
    )
