...
def _parse_section(raw_text, parse_fn, parse_args, validate_fn, validate_args, debug, context):
    """Parse a raw LLM output with non-JSON parser, falling back to JSON extraction."""
    payload, ok = parse_fn(raw_text, *parse_args)
    if not ok:
        json_payload = _parse_json_object(raw_text, debug=debug, context=f"{context}_json_fallback")
...
def _repair_failed_sections(client, parsed, repair_configs, debug):
    ...
    def _repair_task(name, raw_text):
        ...
        text = call_gpt_safe(
            ...
            debug=debug, context=f"synthesize_synopsis:repair:{name}",
...
def _apply_monolith_fallback(client, narrative_text, highlight_min, highlight_max,
                             highlight_label, timeline_min, timeline_max,
                             timeline_label, safe_prompt_kwargs, debug):
    ...
    monolith_prompt = _build_monolith_prompt(
        ...
        timeline_min=timeline_min, timeline_max=timeline_max, timeline_label=timeline_label,
    )
    synopsis_text = call_gpt_safe(
        client, prompt=monolith_prompt, fallback_text=monolith_fallback,
        debug=debug, context="synthesize_synopsis:monolith_fallback",
        safe_prompt=_build_safe_section_prompt(section="monolith", narrative_text=narrative_text, **safe_prompt_kwargs),
...
def _fill_missing_generated(client, generated_questions, narrative_text,
                             extra_questions_count, debug):
    ...
    fill_text = call_gpt_safe(
        client, prompt=fill_prompt, fallback_text=_SECTION_FALLBACKS["qna_generated"],
        debug=debug, context="synthesize_synopsis:fill_generated",
    ...
    fill_questions = _normalize_generated_questions(
        _extract_questions(fill_payload), REQUIRED_QUESTIONS, missing,
        pad=False, exclude_questions=existing,
    )
...
def _run_consistency_pass(client, draft_synopsis, narrative_text, highlight_min,
                          highlight_max, highlight_label, timeline_min, timeline_max,
                          timeline_label, required_block, extra_questions_count, debug):
    ...
    try:
        ...
        consistency_text = call_gpt_safe(
            client, consistency_prompt,
            fallback_text=json.dumps(draft_synopsis, ensure_ascii=False),
            debug=debug, context="synthesize_synopsis:consistency_pass",
            safe_prompt=(
        ...
        return {
            "chat_name": c_chat_name, "summary": c_summary,
            "video_highlights": c_highlights, "video_timeline": c_timeline,
...
def synthesize_synopsis(
    ...
    section_parse_configs = {
        ...
        "highlights": (_parse_highlights_nonjson, (highlight_min, highlight_max), _validate_highlights_payload, (highlight_min, highlight_max)),
        "timeline": (_parse_timeline_nonjson, (timeline_min, timeline_max), _validate_timeline_payload, (timeline_min, timeline_max)),
        "qna_predefined_a": (_parse_questions_nonjson, (len(required_questions_a),), _validate_questions_payload, (len(required_questions_a),)),
        "qna_predefined_b": (_parse_questions_nonjson, (len(required_questions_b),), _validate_questions_payload, (len(required_questions_b),)),
        "qna_generated": (_parse_questions_nonjson, (extra_questions_count,), _validate_questions_payload, (extra_questions_count,)),
    }
    parsed = {}
    for name, (parse_fn, parse_args, validate_fn, validate_args) in section_parse_configs.items():
        raw_text = raw_outputs.get(name, "")
        payload, ok = _parse_section(raw_text, parse_fn, parse_args, validate_fn, validate_args, debug, name)
    ...
    if not (summary_ok and highlights_ok and timeline_ok):
        had_errors = True
        summary_payload, highlights_payload, timeline_payload = _apply_monolith_fallback(
            client, narrative_text, highlight_min, highlight_max, highlight_label,
            timeline_min, timeline_max, timeline_label, safe_prompt_kwargs, debug,
        )
    ...
    predefined_questions = _normalize_predefined_questions(
        _extract_questions(parsed["qna_predefined_a"][0]), required_questions_a,
    ) + _normalize_predefined_questions(
        _extract_questions(parsed["qna_predefined_b"][0]), required_questions_b,
    )
    generated_questions = _normalize_generated_questions(
        _extract_questions(parsed["qna_generated"][0]), REQUIRED_QUESTIONS,
        extra_questions_count, pad=False,
    )
    ...
    if _count_questions(questions) < required_total:
        ...
        questions_text = call_gpt_safe(
            client, questions_prompt, fallback_text=_SECTION_FALLBACKS["qna_generated"],
            debug=debug, context="synthesize_synopsis:legacy_questions",
            safe_prompt=_build_safe_section_prompt(section="qna_legacy", narrative_text=narrative_text, **safe_prompt_kwargs),
            raw_fallback=_SECTION_FALLBACKS["qna_generated"],
        )
    ...
    draft_synopsis = {
        "chat_name": chat_name, "summary": summary,
        "video_highlights": video_highlights, "video_timeline": video_timeline,
        "questions": questions,
    }
    ...
    if mode == "always" or (mode == "on_error" and had_errors):
        synopsis_json = _run_consistency_pass(
            client, draft_synopsis, narrative_text, highlight_min, highlight_max,
            highlight_label, timeline_min, timeline_max, timeline_label,
            required_block, extra_questions_count, debug,
        )
